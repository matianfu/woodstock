...
class DBusDriver extends EventEmitter {
        ...
        if (/^OK\s[a-f0-9]{32}$/.test(s)) {
          ...
          this.invoke({
            ...
            if (err) {
              ...
              console.log('hello replied', body)
  ...
  */
  ...
  send (m) {
    ...
    if (m.type === 'METHOD_RETURN') print(wired)
    return serial
  }
...
{ type: 'METHOD_CALL',
  ...
  version: 1,
  serial: 663,
  path: '/org/bluez/winas/le/advertisement0',
  destination: ':1.871',
  interface: 'org.freedesktop.DBus.Properties',
  member: 'GetAll',
  signature: 's',
  sender: ':1.6',
  body: [ STRING { value: 'org.bluez.LEAdvertisement1' } ] } */

      if (m.interface === 'org.freedesktop.DBus.Properties' &&
        m.member === 'GetAll' &&
        m.path === '/org/bluez/winas/le/advertisement0' &&
        Array.isArray(m.body) &&
        m.body[0] instanceof STRING &&
        m.body[0].value === 'org.bluez.LEAdvertisement1') {
