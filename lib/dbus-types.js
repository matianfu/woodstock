  ...
  constructor (...args) {
    ...
    if (this.constructor === TYPE) { // abstract factory
      ...
      if (!args[1]) {
        ...
        if (Type === VARIANT) {
          throw new Error('VARIANT type cannot be constructed by new TYPE')
        } else if (Type.prototype instanceof CONTAINER_TYPE) {
...
const VARIANT = DEC({ code: 'v', align: 1 })(
  ...
  class VARIANT extends CONTAINER_TYPE {
    ...
    constructor (...args) {
      if (args.length === 0) {
        ...
        if (args.length === 1) {
          ...
          throw new Error('invalid VARIANT args')
        }
      }
    }
    ...
    constructByValues (sig, vals) {
      throw new Error('should variant be constructed by values?')
    }
