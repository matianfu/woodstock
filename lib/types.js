...
class CONTAINER_TYPE extends TYPE {
  ...
  constructor (...args) {
    ...
    if (args.length === 1) {
      if (Array.isArray(args[0])) {
        ...
        this.setSignature(args[0])
      } else {
        throw new Error('invalid elememts nor signature')
      }
    } else if (args.length === 2) {
      this.setElements(args[0])
      this.setSignature(args[1])
    }
  }

  setSignature (sig) {
    if (typeof sig !== 'string') throw new Error('signature not a string')
    this.sig = sig
  }

  setElements (elems) {
    if (!Array.isArray(elems) ||
      !elems.every(el => el instanceof TYPE)) throw new Error('invalid elems')
    this.elems = elems
  }
  ...
  marshal (buf, offset, le) {
    offset = round(offset, this._align)
    return this.elems.reduce((offset, el) =>
...
const STRUCT = DEC({ code: '(', align: 8 })(
  class STRUCT extends CONTAINER_TYPE {
    constructor (...args) {
      super(...args)
      if (this.sig) {
        if (!this.sig || this.sig.length <= 2) throw new Error('invalid sig')
        if (!/^\(.+\)$/.test(this.sig)) throw new Error('invalid sig')
...
const ARRAY = DEC({ code: 'a', align: 4 })(
  class ARRAY extends CONTAINER_TYPE {
    constructor (...args) {
      super(...args)
      if (this.sig) {
        if (this.sig[0] !== 'a') throw new Error('not a struct signature')
    ...
    marshal (buf, offset, le) {
      ...
      if (this.elems.length === 0) {
        ...
        offset += 4
        offset += 4 // TODO this is dirty and buggy!!!

        logm($, this.constructor.name, `${$0}/${$1} { } @${offset}`)

        return offset
      }

      let numOffset = offset
      let elemOffset = round(numOffset, typeMap[this.esig[0]].align)
     
      logm($, this.constructor.name, `${$0}/${$1}, es: ${elemOffset} {`) 
      $more()
...
// TODO this should inherit STRUCT ???
...
/**
      if (sig) {
        ...
        if (this.elems.length !== 2) throw new Error('dict entry requires 2 elements')
        if (!(this.elems[0] instanceof BASIC_TYPE)) {
          throw new Error('dict entry key must be basic type')
        }
      }
*/
    }
