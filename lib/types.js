...
class CONTAINER_TYPE extends TYPE {
  ...
  // new CONTAINER(vals, signature) constructs an object loaded with elements converted from vals, signature is mandatory.
  ...
  constructor (...args) {
    ...
    if (args.length === 1) {  // elems or signature
      ...
      if (args[0].every(e => e instanceof TYPE)) {  // elements
        this.elems = args[0]
        ...
        this.sig = args[1]
      } else if (args[0].every(e => !(e instanceof TYPE))) { // values
...
/**
      if (args.length === 2 && 
        ...
        args[0].every(a => !(a instanceof TYPE)) &&
        args[1] && 
        typeof args[1] === 'string') {
        let esigs = explode(args[1].slice(1, args[1].length - 1))
        super(args.
      } else {
        super(...args)
      }
